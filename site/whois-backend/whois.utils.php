...
class utils extends Whois {
	...
	function showHTML($result, $link_myself=true, $params='query=$0&amp;output=nice') {
        ...
        if($result['regyinfo']['registrar'] === "Nominet UK"){
            // .uk WHOIS is weird, for some reason
            // bold and code tags around ns and other info don't work properly,
            // hence removed <jacob@eml.pm>
        }else{

		    // Add bold field names
		    $out = preg_replace ("/(?m)^([-\s\.&;'\w\t\(\)\/]+:\s*)/", '<strong>$1</strong>', $out);
		    
            // <code> around nservers
            // Jacob Sammon <jacob@eml.pm>
		...
		// ARIN uses #
		$out = preg_replace ("/(?m)^(#.*)/", '<i class="text-muted">$0</i>', $out);
		// Most others use % (inc. AFNIC)
		$out = preg_replace ("/(?m)^(%.*)/", '<i class="text-muted">$0</i>', $out);
		return str_replace("\n","<br/>\n",$out);
	}
}
